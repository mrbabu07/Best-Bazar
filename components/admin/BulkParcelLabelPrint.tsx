...
export function BulkParcelLabelPrint() {
  const printSelected = () => {
    ...
    const markup = labels.map((order) => {
      ...
      return `<section class="label"><header><strong>BEST MART</strong><p>Tracking: ${escapeHtml(order.orderNumber)}</p></header><div class="details"><p><b>Order:</b> #${escapeHtml(order.orderNumber)}<br><b>Date:</b> ${escapeHtml(order.date)}</p><p><b>${escapeHtml(order.customerName)}</b><br>${escapeHtml(order.phone)}<br>${escapeHtml(order.address)}</p></div><div class="bottom"><div><p><b>Products</b><br>${escapeHtml(order.products)}</p><p><b>Payment:</b> ${escapeHtml(order.payment)}<br><b>Due:</b> ${escapeHtml(order.due)}${order.note ? `<br><b>Note:</b> ${escapeHtml(order.note)}` : ""}</p></div><img src="https://api.qrserver.com/v1/create-qr-code/?size=600x600&ecc=L&margin=8&data=${encodeURIComponent(qrText)}" alt="Order QR"></div><footer>Powered by Best Mart Delivery System</footer></section>`;
    }).join("");
