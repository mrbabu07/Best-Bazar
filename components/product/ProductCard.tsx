...
export function ProductCard({ product, locale, dictionary, priority = false, showQuickAdd = true }: ProductCardProps) {
  ...
  return (
    <article className="group relative flex min-w-0 flex-col bg-transparent">
      <div className="relative aspect-[0.72] shrink-0 overflow-hidden rounded-lg bg-[#e9e5dd] sm:aspect-[0.74]">
        <Link href={`/${locale}/product/${product.slug}`} className="block h-full">
          <Image
            src={cardImage.url}
            alt={cardImage.alt}
            fill
            unoptimized={cardImage.url.includes("res.cloudinary.com")}
          ...
          <div className="absolute bottom-2 left-2 flex flex-wrap gap-1.5 rtl:left-auto rtl:right-2">
            ...
            {product.freeDelivery ? <span className="rounded-full bg-emerald-700 px-3 py-1 text-[11px] font-semibold text-white">{locale === "ar" ? "توصيل مجاني" : "Free delivery"}</span> : null}
      ...
      <div className="flex min-h-0 flex-1 flex-col px-0.5 pt-3 text-center">
        <Link
          href={`/${locale}/product/${product.slug}`}
          className="font-editorial line-clamp-2 min-h-[40px] text-[15px] font-semibold leading-5 text-neutral-950 transition hover:underline sm:min-h-[44px] sm:text-[17px] sm:leading-6"
        >
          {productName}
        </Link>
