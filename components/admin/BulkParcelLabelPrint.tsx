...
export function BulkParcelLabelPrint() {
  const printSelected = () => {
    ...
    const markup = labels.map((order) => {
      ...
      return `<section class="label"><header><strong>BEST MART</strong><span>DELIVERY</span></header><section class="recipient"><h1>${escapeHtml(order.customerName)}</h1><p>${escapeHtml(order.address)}</p><b>${escapeHtml(order.phone)}</b><small>ORDER ${escapeHtml(order.orderNumber)}</small></section><p class="date">${escapeHtml(order.date)}</p><section class="product"><b>RACK PRODUCT CODE</b><h2>${escapeHtml(order.productCode)}</h2><p>${escapeHtml(order.products)}</p></section><section class="operations"><div class="codes"><div><small>PRODUCT</small><strong>${escapeHtml(order.subtotal)}</strong></div><div><small>DELIVERY</small><strong>${escapeHtml(order.deliveryFee)}</strong></div><div><small>TOTAL</small><strong>${escapeHtml(order.total)}</strong></div><div><small>PAYMENT</small><strong>${escapeHtml(order.payment)}</strong></div></div><img src="https://api.qrserver.com/v1/create-qr-code/?size=600x600&ecc=M&margin=6&data=${encodeURIComponent(qrText)}" alt="Order QR"></section><footer>Scan QR for customer, order and WhatsApp contact</footer></section>`;
    }).join("");
    const printWindow = window.open("", "_blank", "width=480,height=720");
    if (!printWindow) return;
    printWindow.document.write(`<!doctype html><html><head><title>Parcel labels</title><style>@page{size:4in 6in;margin:0}*{box-sizing:border-box}body{margin:0;font-family:Arial;color:#000}.label{width:4in;height:6in;padding:.15in;border:2px solid #000;page-break-after:always;overflow:hidden}.label:last-child{page-break-after:auto}header{display:flex;align-items:flex-end;justify-content:space-between;border-bottom:3px solid #000;padding-bottom:6px}header strong{font-size:32px;font-weight:900;letter-spacing:-1px}header span{font-size:10px;font-weight:900;letter-spacing:2px}.recipient{padding:10px 0;border-bottom:1px solid #000}.recipient h1{margin:0 0 3px;font-size:20px}.recipient p{margin:0 0 4px;font-size:12px;line-height:1.35}.recipient b{display:block;font-size:13px}.recipient small{display:block;margin-top:7px;font-weight:800;letter-spacing:1px}.date{height:.45in;margin:0;padding:10px 0;border-bottom:2px dashed #000;font-size:13px;font-weight:800}.product{min-height:1.05in;padding:9px 0;border-bottom:2px dashed #000}.product>b{font-size:9px;letter-spacing:1.4px}.product h2{margin:3px 0;font-size:21px;letter-spacing:1px}.product p{margin:5px 0 0;font-size:11px;font-weight:700;line-height:1.4}.operations{display:grid;grid-template-columns:1fr 1.75in;min-height:1.8in}.codes{display:grid;grid-template-columns:1fr 1fr;align-content:start;border-right:2px dashed #000}.codes>div{min-height:.55in;padding:7px 5px;border-bottom:1px dashed #000}.codes small{display:block;margin-bottom:3px;font-size:8px;font-weight:800;letter-spacing:1px}.codes strong{font-size:12px}.operations img{width:1.65in;height:1.65in;margin:auto;border:1px solid #000}footer{border-top:1px solid #000;padding-top:5px;text-align:center;font-size:8px;font-weight:bold}</style></head><body>${markup}</body></html>`);
