...
async function HomepageSectionRenderer({ locale, sections }: { locale: Locale; sections: HomepageSection[] }) {
  ...
  return <>{await Promise.all(visibleSections.map(async (section) => {
    ...
    if (section.type === "CATEGORY_PRODUCT_ROWS") {
      ...
      return <section key={section.id} className="bg-white py-12 sm:py-16"><div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">{rows.map(({ category, products }) => products.length ? <div key={category.id} className="mb-14 last:mb-0"><SectionHeader title={getLocalized(category.name, locale)} action={<Link href={`/${locale}/shop?category=${category.slug}`} className="text-sm font-medium text-neutral-900 underline underline-offset-4">{section.config.actionLabelEn || "View all"}</Link>} /><div className="mt-8 grid grid-cols-2 gap-x-3 gap-y-8 sm:grid-cols-3 sm:gap-x-4 lg:grid-cols-4 xl:grid-cols-5">{products.map((product, index) => <ProductCard key={product.id} product={product} locale={locale} dictionary={dictionary} priority={index < 2} />)}</div></div> : null)}</div></section>;
    }
